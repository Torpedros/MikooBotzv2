...
const jadibot = async (Senchini, m, from) => {
      ...
      client[from].ev.on("connection.update", async up => {
        ...
        if (connection === "close") {
          if (reason === DisconnectReason.restartRequired) {
            ...
            return Senchini.sendMessage(from, { text: "Anda sudah tidak lagi menjadi bot." })
          }
        }
      })
...
async function stopjadibot(Senchini, from) {
  if (!Object.keys(client).includes(from)) {
    return Senchini.sendMessage(from, { text: "Anda tidak ada di list jadi bot." })
  }
  try {
...
async function listjadibot(Senchini, m) {
  ...
  return Senchini.sendMessage(from, { text: text.trim(), mentions, })
}
