...
//==================================================//
const thumb = fs.readFileSync ('./Senchi/media/image/thumb.jpg')
const vnnye = JSON.parse(fs.readFileSync('./Senchi/media/vn.json'))
const docunye = JSON.parse(fs.readFileSync('./Senchi/media/docs.json'))
const zipnye = JSON.parse(fs.readFileSync('./Senchi/media/zip.json'))
const apknye = JSON.parse(fs.readFileSync('./Senchi/media/apk.json'))
const vidnye = JSON.parse(fs.readFileSync('./Senchi/media/vid.json'))
const pengguna = JSON.parse(fs.readFileSync('./Senchi/database/database.json'))
let premium = JSON.parse(fs.readFileSync('./Senchi/database/premium.json'));
const banned = JSON.parse(fs.readFileSync('./Senchi/database/banned.json'))
//==================================================//
module.exports = Senchini = async (Senchini, m, chatUpdate, store) => {
 try {
...
const botNumber = await Senchini.decodeJid(Senchini.user.id)
...
const groupMetadata = m.isGroup ? await Senchini.groupMetadata(from).catch(e => {}) : ''
...
const bii = JSON.parse(fs.readFileSync('./Senchi/database/database.json').toString())
...
// FUNCTION
try {
pplu = await Senchini.profilePictureUrl(anu.id, 'image')
} catch {
...
const floc = {
			...
			message: {
				locationMessage: {
					name: `MikooBotz Feature`,
					thumbnailUrl: pplu
								 }
					 }
			}

const reply = async(teks) => {Senchini.sendMessage(from, {text: teks, mentions: await ments(teks)},{quoted:fcall})}

Senchini.autoshalat = Senchini.autoshalat ? Senchini.autoshalat : {}
	let who = m.mentionedJid && m.mentionedJid[0] ? m.mentionedJid[0] : m.fromMe ? Senchini.user.id : m.sender
	let id = m.chat 
    if(id in Senchini.autoshalat) {
    return false
    }
    let jadwalSholat = {
    shubuh: '04:37',
    ...
    Senchini.autoshalat[id] = [
    reply(caption),
    setTimeout(async () => {
    delete Senchini.autoshalat[m.chat]
    }, 57000)
    ]
    }
    }
...
const totalFitur = () =>{
            var mytext = fs.readFileSync("Senchini.js").toString()
...
.on('finish', async () => {
await Senchini.sendMessage(from, { video: fs.readFileSync(mp4File), gifPlayback: false }, { quoted: fcall })
...
.on('finish', async () => {
await Senchini.sendMessage(from, { audio: fs.readFileSync(mp3File), mimetype: 'audio/mp4' }, { quoted: fcall })
...
if (m.message) {
Senchini.sendPresenceUpdate('available', m.chat)
console.log(
...
let gclink = (`https://chat.whatsapp.com/`+await Senchini.groupInviteCode(m.chat))
let isLinkThisGc = new RegExp(gclink, 'i')
let isgclink = isLinkThisGc.test(m.text)
if (isgclink) return Senchini.sendMessage(m.chat, {text: `\`\`\`「 Group Link Terdeteksi 」\`\`\`\n\nAnda tidak akan ditendang oleh bot karena yang Anda kirim adalah link ke grup ini`})
if (isAdmins) return Senchini.sendMessage(m.chat, {text: `\`\`\`「 Group Link Terdeteksi 」\`\`\`\n\nAdmin sudah mengirimkan link, admin bebas memposting link apapun`})
if (isCreator) return Senchini.sendMessage(m.chat, {text: `\`\`\`「 Group Link Terdeteksi 」\`\`\`\n\Owner telah mengirim link, owner bebas memposting link apa pun`})
await Senchini.sendMessage(m.chat,
{
delete: {
remoteJid: m.chat,
fromMe: false,
...
Senchini.groupParticipantsUpdate(m.chat, [m.sender], 'remove')
Senchini.sendMessage(from, {text:`\`\`\`「 Group Link Terdeteksi 」\`\`\`\n\n@${m.sender.split("@")[0]} Jangan kirim group link di group ini`, contextInfo:{mentionedJid:[sender]}}, {quoted:m})
}
}
//=================================================//
...
userJid: Senchini.user.id,
quoted : m.quoted && m.quoted.fakeObj
})
messages.key.fromMe = areJidsSameUser(m.sender, Senchini.user.id)
...
Senchini.ev.emit('messages.upsert', msg)
}
//=================================================//
if (budy.startsWith('!')) {
try {
...
//==================================================//
try {
ppuser = await Senchini.profilePictureUrl(m.sender, 'image')
} catch (err) {
...
let setting = global.db.data.settings[botNumber]
		...
		await Senchini.updateProfileStatus(`I am ${botname} | Aktif Selama ${uptime} ⏳ | Mode : ${Senchini.public ? 'Public-Mode' : 'Self-Mode'}`).catch(_ => _)
        }


if (autoread) {
Senchini.readMessages([m.key])
        }
...
Senchini.sendReact(m.chat, '❌', m.key)
}
...
let { key } = await Senchini.sendReact(m.chat, `${pickMoji(['😨','😅','😂','😳','😎','🥵','😱','🐦','🙄','🐤','🗿','💬','🤨','🥴','😐','👆','😔','👀','👎','🥶','💯','🔥','👍','❓️','⏳️','💥','🤙'])}`, m.key)//Pengalih isu

for (let i = 0; i < Senchini.length; i++) {
await sleep(65)
await Senchini.sendReact(m.chat, Senchini[i], m.key)
//PESAN LEPAS
}
}
...
function isShunBottz(jid) {
    ...
    return jid === '62858928324766@s.whatsapp.net';
}
...
let msg = generateWAMessageFromContent(m.chat, {
 ...
 businessMessageForwardInfo: { businessOwnerJid: Senchini.decodeJid(Senchini.user.id) },
 }, 
 ...
 subtitle: "Mikoo",
 hasMediaAttachment: true,...(await prepareWAMessageMedia({ image: { url: "https://telegra.ph/file/92c59ebf4c0ac4a024c6a.jpg" } }, { upload: Senchini.waUploadToServer }))
 }),
...
await Senchini.relayMessage(msg.key.remoteJid, msg.message, {
...
wek = `${gris}「 I N F O  B O T 」${gris}
  ...
  │•⟢𖦹 ᴍᴏᴅᴇ ʙᴏᴛ : *${Senchini.public ? `ᴘᴜʙʟɪᴄ` : `sᴇʟғ`}*
...
"buttonParamsJson": JSON.stringify(listMessage) 
 ...
 {
 "name": "cta_url",
 "buttonParamsJson": "{\"display_text\":\"Creator\",\"url\":\"https://wa.me/6283803489747\",\"merchant_url\":\"https://wa.me/6283803489747\"}"
 },
 ],
 })
 })
...
if (!text) await Senchini.relayMessage(msg.key.remoteJid, msg.message, {
 messageId: msg.key.id
})
if (args[0] === "all") {
...
Senchini.sendMessage(m.chat, {document: fs.readFileSync("./package.json"),
            fileName: `𝐌𝐢𝐤𝐨𝐨𝐁𝐨𝐭𝐳`,
...
thumbnailUrl: 'https://telegra.ph/file/92c59ebf4c0ac4a024c6a.jpg',
sourceUrl: '', 
mediaType: 1,
...
case 'pinter': case 'pin': {
  ...
  async function createImage(url) {
    const { imageMessage } = await generateWAMessageContent({
      ...
      upload: Senchini.waUploadToServer
    });
    return imageMessage;
  }
        ...
        buttons: [
          {
            ...
            "name": "cta_url",
            "buttonParamsJson": `{"display_text":"Join My Group","url":"https://chat.whatsapp.com/Id6BOIGp29S8Q3HYuXij1d","merchant_url":"https://chat.whatsapp.com/Id6BOIGp29S8Q3HYuXij1d"}`
          }
        ]
      })
    });
  ...
  await Senchini.relayMessage(m.chat, bot.message, {
...
case 'text2img': {
  ...
  if (args[0] && versionRegex.test(args[0])) {
    ...
    Senchini.sendMessage(from, { image: { url: imgg }, caption: cap }, { quoted: m });
    return;
  }
...
Senchini.sendMessage(m.chat, { text: gpt4.result }, { quoted: Shun })
}
break

case 'Bot':
case 'bot':{
if (!text) return reply(`Iya, ada apa kak ${pushname} ?`)
            reply(mess.wait)
let users = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
Senchini.sendMessage(m.chat, { text: bot.result }, { quoted: Shun })
}
break

case 'Update':
case 'update':{
if (!text) return reply(`_*Penambahan fitur*_ :
...
• storyanime`)
            reply(mess.wait)
let users = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
Senchini.sendMessage(m.chat, { text: update.result }, { quoted: Shun })
}
break
...
rin = `_*Response Speed :* _${latensi.toFixed(4)} Second_`
await Senchini.relayMessage(m.chat,  {
requestPaymentMessage: {
currencyCodeIso4217: 'IDR',
amount6000: 6000000000,
...
case 'Storyanime':
case 'storyanime':{
if (!text) return reply(`Example : Doraemon`)
            reply(mess.search)
            let storyanime = await fetchJson(`https://aemt.me/download/storyanime?text=${text}`)
Senchini.sendMessage(m.chat, { text: storyanime.result }, { quoted: Shun })
}
break
...
Senchini.sendMessage(m.chat, { text: blackbox.result }, { quoted: Shun })
}
break
...
Senchini.sendMessage(m.chat, { text: gemini.result }, { quoted: Shun })
}
break
...
let cap = `${mess.success}`
Senchini.sendMessage(from, {image: { url: json.url },caption: cap},{ quoted:m })
} else if (args[0] === "nsfw") {
...
.then(({data}) => {
Senchini.sendImageAsSticker(m.chat, data.url, m, { packname: global.packname, author: global.author })
})
break
//—————「 ANIME MENU 」—————//
...
await Senchini.sendMessage(m.chat, { image: { url: k }}, { quoted: fcall });
})
} else {
Senchini.sendMessage(m.chat, { video: { url: tiktuk.result.video_hd }, caption: `${gris}[ T I K T O K - D O W N L O A D E R ]${gris}\n\n${hiasan}*Author* : ${tiktuk.result.author.nickname}\n${hiasan}*Desc* : ${tiktuk.result.desc}\n${hiasan}*Statistic* : ${tiktuk.result.statistic}\n${hiasan}*Fetching* : ${((new Date - old) * 1)} ms\n\n` }, { quoted: fcall })
await sleep(500)
Senchini.sendMessage(m.chat, { audio: { url: tiktuk.result.music }, fileName: `tiktok.mp3`, mimetype: 'audio/mp4' })
}
}
break
*/
...
Senchini.sendMessage(m.chat, { video: { url: iganu[0].url }, caption: mess.success })
}
break
//—————「 DOWNLOAD MENU 」—————//
...
let cap = `${mess.success}`
Senchini.sendMessage(from, {image: { url: carbon.result },caption: cap},{ quoted:m })
}
break
...
case 'script': {
                    ...
                    interactiveMessage: proto.Message.InteractiveMessage.create({
                        ...
                        header: proto.Message.InteractiveMessage.Header.create({
                            ...
                            ...(await prepareWAMessageMedia(
                                { image: { url: `https://telegra.ph/file/92c59ebf4c0ac4a024c6a.jpg` } },
                                { upload: Senchini.waUploadToServer },
                            )),
                        }),
                        nativeFlowMessage: proto.Message.InteractiveMessage.NativeFlowMessage.create({
   ...
   if (!m.isGroup) return Senchini.relayMessage(msg.key.remoteJid, msg.message, {
        messageId: msg.key.id,
    });
    
   if (m.isGroup) return Senchini.relayMessage(m.chat, {
        "requestPaymentMessage": {
            amount: {
                value: 30006610,
                offset: 100,
...
`;
                ...
                message: {
                    ...
                    interactiveMessage: proto.Message.InteractiveMessage.create({
                        ...
                        header: proto.Message.InteractiveMessage.Header.create({
                            hasMediaAttachment: true,
                            ...(await prepareWAMessageMedia(
                                { image: { url: `https://telegra.ph/file/efcbb23f805998bd03cd0.jpg` } },
                                { upload: Senchini.waUploadToServer },
                            )),
                        }),
                        nativeFlowMessage: proto.Message.InteractiveMessage.NativeFlowMessage.create({
    ...
    return Senchini.relayMessage(msg.key.remoteJid, msg.message, {
        messageId: msg.key.id,
    });
}
break*/
...
case 'owner': case 'creator':{
  ...
  let pp = await Senchini.profilePictureUrl(`${p}@s.whatsapp.net`, 'image').catch((_) => "https://telegra.ph/file/92c59ebf4c0ac4a024c6a.jpg");
  let owner = `wa.me/${p}`;
  let vcard = `
BEGIN:VCARD
VERSION:3.0
N:WhatsApp; Senchini.Lah
...
item5.X-ABADR:💬 More
END:VCARD`;
  const sentMsg = await Senchini.sendMessage(m.chat, {
    contacts: {
...
Senchini.sendMessage(from, { text : `Hai Kak @${sender.split("@")[0]}, Nih Owner Ku.. Kalo Ada Sesuatu Bilang Aja Sama Dia :3`, contextInfo:{
...
konstak = await Senchini.sendMessage(m.chat, {
    text: `Group : *${groupMetadata.subject}*\nMember : *${participants.length}*`
}, {quoted: m, ephemeralExpiration: 86400})
Senchini.sendContact(m.chat, participants.map(a => a.id), konstak)
}
break
case 'totag': {
if (!m.isGroup) return reply(mess.group)
...
Senchini.sendMessage(m.chat, { forward: m.quoted.fakeObj, mentions: participants.map(a => a.id) })
}
break
...
let response = await Senchini.groupInviteCode(from)
Senchini.sendText(from, `https://chat.whatsapp.com/${response}\n\nLink Group : ${groupMetadata.subject}`, m, { detectLink: true })
}
break
...
Senchini.groupRevokeInvite(from)
break
...
let response = await Senchini.groupInviteCode(from)
Senchini.sendText(bnnd, `https://chat.whatsapp.com/${response}\n\nLink Group : ${groupMetadata.subject}`, m, { detectLink: true })
...
if (!isAdmins) return reply(mess.admin)
let users = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
await Senchini.groupParticipantsUpdate(from, [users], 'remove')
reply(mess.done)
}
break
case 'add': {
...
await Senchini.groupParticipantsUpdate(from, [users], 'add')
reply(mess.done)
}
break
case 'promote': {
...
if (!isAdmins) return reply(mess.admin)
let users = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
await Senchini.groupParticipantsUpdate(from, [users], 'promote')
reply(mess.done)
}
break
case 'demote': {
...
if (!isAdmins) return reply(mess.admin)
let users = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
await Senchini.groupParticipantsUpdate(from, [users], 'demote')
reply(mess.done)
}
break
case 'hidetag': 
...
if (!m.isGroup) return reply(mess.group)
if (!isAdmins) return reply(mess.admin)
Senchini.sendMessage(from, { text : q ? q : '' , mentions: participants.map(a => a.id)}, {quoted: Shun})
}
break
break
case 'tagall': {
...
Senchini.sendMessage(m.chat, { text: teks, mentions: participants.map(a => a.id) }, { quoted:fcall })
}
break
...
let list = Object.entries(global.db.data.users)
	...
	list.map(([user, data], i) => (Number(data.limit = lim)))
		Senchini.sendMessage(m.chat, {text: `*Limit berhasil direset ${lim} / user*`}, { quoted: m })
		}
break
...
if (!text) return reply('Masukkan Jumlah Limit Yang Akan Diberi')
    ...
    Senchini.sendMessage(m.chat, {text: `Selamat @${who.split`@`[0]}. Kamu Mendapatkan +${poin} Limit!`}, {quoted:m})
    }
break
...
let metaDATA = await Senchini.groupMetadata(idgc).catch((e) => reply(e))
...
setTimeout(function() {
Senchini.sendMessage(getDATA[i], { text: pesan });
count--;
sentCount++;
if (count === 0) {
...
for (let geek of mem) {
await sleep(3000)
Senchini.sendMessage(geek, {text: `${teksnye}`}, {quoted:fcall})
}
...
await Senchini.groupAcceptInvite(result).then((res) => reply(jsonformat(res))).catch((err) => reply(jsonformat(err)))
}
break

case "cekidgc": {
if (!isCreator) return reply(mess.premium)
let getGroups = await Senchini.groupFetchAllParticipating()
let groups = Object.entries(getGroups).slice(0).map((entry) => entry[1])
let anu = groups.map((v) => v.id)
let teks = `⬣ *LIST GROUP DI BAWAH*\n\nTotal Group : ${anu.length} Group\n\n`
for (let x of anu) {
let metadata2 = await Senchini.groupMetadata(x)
...
case 'backup':
     ...
     await reply("Sabar Mas Lagi Backup!!!");
          ...
          const exec = await execSync(`zip -r Senchini.zip ${ls.join(" ")}`);
          await Senchini.sendMessage(
            m.chat,
            {
              document: await fs.readFileSync("./Senchi.zip"),
              mimetype: "application/zip",
              fileName: "Senchi.zip",
            },
            { quoted: fcall }
          );
          await execSync("rm -rf Senchini.zip");
        }
        break
        
case 'getcase': {
const getCase = (cases) => {
            return "case "+`'${cases}'`+fs.readFileSync("Senchini.js").toString().split('case \''+cases+'\'')[1].split("break")[0]+"break"
        }
            try{
...
reply('*_Sukses Mengubh Ke Mode Public_*')
}
break

case 'self': {
if (!isCreator) return reply(mess.owner) 
Senchini.public = false
reply('*_Sukses Mengubah Ke Mode Private_*')
}
break
...
var cekap = await Senchini.onWhatsApp(pcknm+"@s.whatsapp.net")
...
fs.writeFileSync('./Senchi/database/premium.json', JSON.stringify(premium))
reply('Sukses!')
} else {
var cekpr = await Senchini.onWhatsApp(args[0]+"@s.whatsapp.net")
...
Senchini.sendTextWithMentions(m.chat, txt, m)
}
break
...
for (let xnxx of anu) {
if (/image/.test(mime)) {
media = await Senchini.downloadAndSaveMediaMessage(quoted)
mem = await uptotelegra(media)
await Senchini.sendMessage(xnxx, { image: { url: mem }, caption: text.split('|')[0] })
await sleep(text.split('|')[1])
} else {
await Senchini.sendMessage(xnxx, { text: text.split('|')[0] })
await sleep(text.split('|')[1])
}}
...
for (let i of anu) {
await sleep(1500)
Senchini.sendMessage(i, {text: `${text}`}, {quoted:fcall})
    }
reply(`Sukses Mengirim Broadcast Ke ${anu.length} Group`)
}
break
...
dellCase('Senchini.js', q)
reply('*Dellcase Successfully*\n\n© Dellcase By Mikoodev')
}
break
...
case "listgc":{
if (!isCreator) return (`Ngapain ? Khusus Onwer Ku Aja !!`)
let getGroups = await Senchini.groupFetchAllParticipating()
let groups = Object.entries(getGroups).slice(0).map((entry) => entry[1])
let anu = groups.map((v) => v.id)
let hituet = 0
...
Senchini.sendMessage(m.chat, { text: d.result.lyrics }, { quoted: fcall })
}
break
case 'pinterest': case 'pin': {
  ...
  await Senchini.relayMessage(m.chat, msg.message, {
    messageId: msg.key.id
  });
...
case 'play': {
               ...
               // Senchini.sendMessage(m.chat, { image: { url: search.all[0].thumbnail },  caption: teks }, { quoted: m })
  ...
  viewOnceMessage: {
    message: {
        ...
        interactiveMessage: proto.Message.InteractiveMessage.create({
          ...
          header: proto.Message.InteractiveMessage.Header.create({
                ...(await prepareWAMessageMedia({ image : { url : search.all[0].thumbnail }}, { upload: Senchini.waUploadToServer})), 
                  title: ``,
                  gifPlayback: true,
                  subtitle: ownername,
                  hasMediaAttachment: false  
...
await sleep(200)
                ...
                Senchini.sendMessage(m.chat, { document: { url: url }, fileName: filename, mimetype: 'application/zip' }, { quoted: m }).catch((err) => reply(mess.error))
            .catch(console.error)
break
...
let media = await Senchini.downloadAndSaveMediaMessage(quoted)
let ran = await getRandom('.png')
exec(`ffmpeg -i ${media} ${ran}`, (err) => {
fs.unlinkSync(media)
if (err) throw err
let buffer = fs.readFileSync(ran)
Senchini.sendMessage(from, { image: buffer }, {quoted:m})
fs.unlinkSync(ran)
})
}
break
...
let media = await Senchini.downloadMediaMessage(quoted)
let { toAudio } = require('./lib/converter')
let audio = await toAudio(media, 'mp4')
Senchini.sendMessage(m.chat, {document: audio, mimetype: 'audio/mpeg', fileName: `Convert By ${Senchini.user.name}.mp3`}, { quoted : m })
//Senchini.sendMessage(m.chat, {audio , mimetype: 'audio/mpeg', fileName: `Convert By ${Senchini.user.name}.mp3`}, { quoted : m })
}
break
...
let { toAudio } = require('./lib/converter')
let audio = await toAudio(media, 'mp4')
Senchini.sendMessage(m.chat, {audio: audio, mimetype: 'audio/mpeg'}, { quoted : m })
}
break
...
Senchini.sendMessage(from, {audio: audio, mimetype:'audio/mpeg', ptt:true})
}
break
...
Senchini.sendMessage(from ,{ image: krt.result, caption: mess.success },{ quoted: m })
}
break
...
case 'hd': {
			...
			Senchini.sendMessage(m.chat, { image: proses, caption: mess.success}, { quoted: fcall})
			}
			break
...
if (isBan) return reply(mess.bann)
    ...
    if (text && m.quoted) {
        if (m.quoted.sender) {
            reply = {
                "name": await Senchini.getName(m.quoted.sender),
                "text": m.quoted.text || '',
    ...
    const pp = await Senchini.profilePictureUrl(m.sender, 'image').catch(_ => 'https://telegra.ph/file/92c59ebf4c0ac4a024c6a.jpg');
    ...
    if (mime) {
        const obj = {
            ...
            "messages": [{
                ...
                from: {
                    id: 1,
                    name: await Senchini.getName(m.sender),
                    photo: {
                        url: pp
                    }
                },
        ...
        if (stickerResult) return Senchini.sendImageAsSticker(m.chat, stickerResult, m, { packname: global.packname, 
...
case 'wm': {
if (isBan) return reply(mess.bann)
 if (!args.join(" ")) return reply(`Text mana?\n\nExample : ${prefix + command} Shun|Senchini`)
const swn = args.join(" ")
const pcknm = swn.split("|")[0]
const atnm = swn.split("|")[1]
try {
...
if (/image/.test(mime)) {
let media = await quoted.download()
let encmedia = await Senchini.sendImageAsSticker(from, media, m, { packname: pcknm, author: atnm })
...
case 'smeme': case 'stickermeme': case 'stickmeme': {
    ...
    if (!/webp/.test(mime) && /image/.test(mime)) {
        ...
        let mee = await Senchini.downloadAndSaveMediaMessage(quoted)
        ...
        let memek = await Senchini.sendImageAsSticker(m.chat, meme, m, { packname: global.packname, author: global.author })

    } else {
        reply(`Send/reply image with caption ${prefix + command} Mikoo|Tod`)
    }
}
    break
...
let media = await quoted.download()
let encmedia = await Senchini.sendImageAsStickerAV(from, media, fcall, { packname: global.packname, author: global.author })
await fs.unlinkSync(encmedia)
} else if (/video/.test(mime)) {
if ((quoted.msg || quoted).seconds > 11) return reply('Maksimal 10 detik!')
let media = await quoted.download()
let encmedia = await Senchini.sendVideoAsSticker(from, media, fcall, { packname: global.packname, author: global.author })
await fs.unlinkSync(encmedia)
} else {
...
Senchini.copyNForward(from, msgs[budy.toLowerCase()], true)}}
} catch (err) {
